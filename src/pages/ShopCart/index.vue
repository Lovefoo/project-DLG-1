<template>
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        <div class="chosed">已选择 <span>{{isAllCheckedNum}}</span>件商品</div>
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
          <i class="summoney">{{ totalPrice }}</i>
        </div>
        <div class="sumbtn">
          <button class="sum-btn"  @click="toTrade" v-if="totalPrice">结算</button>
          <button class="sum-btn"  style="background:gray" v-else>结算</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    //判断底部复选框是否勾选 (全部产品都选中，采用勾选)
    isAllChecked() {
      return this.cartInfoList.every((item) => item.isChecked == 1);
    },
    //判断选中了几样商品
    isAllCheckedNum(){
      return this.cartInfoList.reduce((num,item)=>{
        if(item.isChecked==1){
          return num+1;
        }else{
          return num;
