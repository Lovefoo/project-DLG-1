            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="skuNum > 1 ? skuNum-- : null"
                  >-</a
                >
              </div>
              <div class="add">
                <!-- 以前的路由跳转，从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求，把你购买产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    skuAttribute(){
      ...
      this.spuSaleAttrList.forEach((item)=>{
        obj[item.saleAttrName] = item.spuSaleAttrValueList.filter(item => item.isChecked==1)[0].saleAttrValueName;
      });
      return obj;
    }
  },
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      ...
      // this.skuAttribute[spuSaleAttrValue.saleAttrName] = spuSaleAttrValue.saleAttrValueName;
    ...
    async addShopCar() {
      ...
      /* 
        当前这里是派发一个action，也向服务器发请求，判断加入购物车是成功还是失败了 
        this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum});
        上面这行代码: 调用仓库中的addOrUpdateShopCart，这个方法加上async，返回的一定是一个Promise
        要么成功 | 要么失败
      */
      //2.服务器存储成功---进行路由跳转传递参数，如果失败，给用户进行提示
      try {
        ...
        //不管本地存储 还是 会话存储，一般存储的是字符串
        sessionStorage.setItem('skuInfo',JSON.stringify(this.skuInfo));
        sessionStorage.setItem('skuAttribute',JSON.stringify(this.skuAttribute));
